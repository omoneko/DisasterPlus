...
**★★ 発生地点と強度はプレイヤーが指す（2026-08-20 / 2026-08-21 変更）。**

災害パネルの④タイルは、バニラの災害ボタンと**同じ 3 手**である
（所有者の指示「ほかの災害と同じようにタブから選択してスケール選択して
発生個所押したらその災害が起こるように」）:

| | 手 | 起きること |
|---|---|---|
| 1 | ④タイルを押す | 配置カーソルが構わり、**バニラの強度スライダーが出る** |
| 2 | スライダーを動かす | 台風の強度（バニラの `m_intensity` と同じ意味・同じ尺度） |
| 3 | 地図をクリック | その地点から、**そのとき選ばれていた強度**で始まる |

地図をクリックした地点が台風の**出発点**である（`TyphoonPlacementTool` →
`TyphoonRequestData` が座標と強度を運ぶ → `TyphoonController.Start(…, origin, intensity)`）。

**★★ タイルは説明のパネルを開かない。** 以前は「カーソルを構える＋パネルを開く」
だったが、所有者の指示「あれこれ説明は出さなくていい」により、タイルは
**起こすためだけのもの**になった。④の状態と説明は**左上のショートカット
（`InfoHub`）→「台風」タブ**から読む。パネルの中の「発生させる」ボタンも撤去し、
起こす経路をタイル 1 つに閉じた（「止める」は残る）。
...
**★ 設定画面の「台風の強度」は死んでいない。** タイルを構えるたびに
その値がスライダーの初期値として入る（`IntensitySlider.Seed`）ので、
「いつもの強さ」を決めるつまみとして生きている。読めない環境では
その値がそのまま使われる。
...
#### ★ 追加（2026-08-22、持ち主の指摘「暴風雨を再現してほしいです」）

④が「嵐の強さ」として出せていたのは `m_targetRain` / `m_targetCloud` の 2 つだけで、
どちらも**空全体の設定**である。バニラの雨は真下に降り、風向きにも台風の位置にも従わない。
地上のプレイヤーから見ると、最盛期の台風もただの雨と同じ絵になっていた。

**雨量はもう上げられない。** 最盛期の `m_targetRain` は既に 1.0 で、しかも
`m_currentRain > 0.8` はゲーム自身に雷雨災害を作らせる境界である（§A-3）。
④の落雷の予算はその手前で釣り合いを取っているので、**ここは 1 バイトも動かさない。**

代わりに 3 つ足した。**どれも建物・道路・樹木には触れない**（風害とは別の設定
`TyphoonStormFx` で入り切りする）:

| | 何を | どこで | なぜ雨量の釣り合いを壊さないか |
|---|---|---|---|
| 1 | **横殴りの飛沫**（`TyphoonSquallFx` + `Core/Typhoon/SquallLayout`） | main スレッド。**カメラの真下の地面**に 9 か所、風下へ向かう `velocity` を持った水の粒 | 描画だけ。シミュレーションの状態を 1 バイトも変えない |
| 2 | **視程が落ちる**（`TyphoonWeather` が `m_targetFog` を 0 → 最大 0.45） | sim スレッド、毎 tick | `QueueLightningStrike` の条件は `m_currentRain > 0.8` **だけ**で、`m_currentFog` はどこにも出てこない（IL 再確認） |
| 3 | **突風が市民と車を飛ばす**（`TyphoonWind.Gale`） | sim スレッド、64 フレームごと | `DisasterHelpers.AddWind` は `AddWindCitizens` + `AddWindVehicles` の 2 行だけ（§B-1） |

3 の費用は**下がっている**: 風害の走査の中の押しは強風域（暴風域の 2.2 倍）で撃っていたが、
新しい突風は**暴風域**（面積で 1/4.84）なので、4 倍の頻度でも合計は 0.83 倍にしかならない。
実在の台風でも最も強い風は眼の壁雲の周りにある。

飛沫は**カメラが強風域の外に居ると 1 粒も出ない**（`SquallLayout.StrengthOf` が
`WindAt` の風速相当 0.18 未満で 0 を返す）。これは不具合ではなく、診断ダンプが
`the camera is outside the storm` と名乗る。

見え方は `tools/TyphoonPreview` の `squall-elevation.png` でオフラインに確かめてある。

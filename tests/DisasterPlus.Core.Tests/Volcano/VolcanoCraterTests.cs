...
{
    /// <summary>
    /// That the crater is **part of the height profile** (in-game point ①).
    ///
    /// Four things are pinned here:
    ///   1. Outside radius R it is exactly 0 (do not raise ground the preparation has not
    ///      reached. Design doc §1.2)
    ///   2. It never exceeds the final height H, not even by a millimetre (ceiling §C-10)
    ///   3. **The crater is genuinely a depression** —— the centre is lower than the rim.
    ///      With a naive subtraction this inverts for a stratovolcano
    ///      (class doc of <see cref="VolcanoCrater"/>)
    ///   4. **It is a depression from the very start of the uplift** —— as progress rises,
    ///      the difference between rim and floor increases monotonically and tops out at
    ///      depth (it never decreases)
    /// </summary>
    public class VolcanoCraterTests
    {
        ...
        {
            // With relief 0 (= a smooth cone), the crater rim reaches exactly H.
            for (int i = 0; i < AllForms.Length; i++)
            {
                VolcanoForm form = AllForms[i];
                float r = VolcanoShape.DefaultRadiusOf(form);
        ...
        {
            // ★ This is the heart of point ①. Naively "subtracting a hollow from the cone"
            //   makes the centre end up higher on a stratovolcano (the cone drops 72 m over
            //   the crater radius, while the crater is only 60 m deep).
            for (int i = 0; i < AllForms.Length; i++)
            {
                VolcanoForm form = AllForms[i];
                float r = VolcanoShape.DefaultRadiusOf(form);
        ...
        {
            // The flames from the vent are a disc with a radius, so if the floor is a bowl
            // they sink into the ground at its edge.
        ...
        {
            // The uplift writes max(0, profile - H(1-p)). The difference between rim and
            // floor increases monotonically with p and tops out at depth = "it exists as a
            // depression from the very beginning".
            const VolcanoForm form = VolcanoForm.Strato;
            float r = VolcanoShape.DefaultRadiusOf(form);
            float h = VolcanoShape.DefaultHeightOf(form);
            var relief = VolcanoRelief.For(form, Seed, 0f);
        ...
        public void TheConeIsRaisedJustEnoughForTheRimToReachTheHeight()
        {
            ...
            // For a stratovolcano cr = 0.12R holds at any radius, so the factor is constant.
        ...
        {
            // At a factor of 1 it is just the progress, as before.
            ...
            // With the rebuilt cone the front runs ahead (= the preparation is run that far
            // ahead too).
            ...
            // Even with bad input it never leaves [0,1].

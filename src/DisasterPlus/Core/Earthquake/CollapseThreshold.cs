namespace DisasterPlus.Core.Earthquake
{
    ...
    public static class CollapseThreshold
    {
        ...
        /// <summary>
        /// 全体円盤の probability（IL: ldc.r4 0.02）。震央で 2%。
        ///
        /// **この値は Natural Disasters Renewal の識別子でもある**（IL 事実文書 §E-2）。
        /// NDR は <c>DisasterHelpers.DestroyBuildings</c> を Prefix が false を返す形で
        /// **完全置換**し、<c>probability == 0.02f</c> をバニラ地震の目印にして
        /// 0.04 に差し替える。したがって NDR が入っている環境では、この定数から
        /// 導いた倒壊・出火の結論は**バニラの答えでも NDR の答えでもない**。
        /// <see cref="BuildingMargin.Evaluate"/> の <c>damageModelReplaced</c> を
        /// 立てて、判定そのものを取り下げること。
        /// </summary>
        ...
        ///
        /// なお断層 4 円盤ではこの比較を**呼ぶ意味自体が無い**。<c>preRadius = w</c> が
        /// <c>dist &gt;= w</c> を先に切り捨て、残った範囲では fD &gt; 1 かつ
        /// probability = 1 なので比較は常に真になる（<see cref="FaultBand"/> の doc）。
        /// </summary>
        ...
        /// <see cref="FaultBand.PatchRadiusAt"/> を使うこと（あちらは別のモデルである）。
        /// </summary>
        ...
        /// <summary>
        /// **出火**が始まる震央距離。全体円盤では
        /// <c>burnRadiusMin = 0, burnRadiusMax = R</c>（§A-3）なので
        /// <c>fB = 1 - d/R</c> となり、倒壊のランプと**同じ式**になる。
        /// 違うのは引くしきい値（2 回目の引き）だけである。
        ///
        /// **倒壊が優先する。** IL は <c>else if (hitB &amp;&amp; ...)</c> なので、
        /// 同じ建物で倒壊も当たっているならこちらの分岐へは来ない
        ...
        /// <summary>
        /// 全体円盤のランプ（<c>1 - d/R</c>）と probability 0.02 に対して、
        /// しきい値 <paramref name="threshold"/> が当たり始める震央距離。
        ///
        ///   threshold &lt; (1 - d/R) * 0.02 * 10000
        ///   d &lt; R * (1 - threshold / (0.02 * 10000))
        /// </summary>

...
{
    /// <summary>
    /// 火山パネルを開閉するトグルボタン。①の <see cref="ForecastPanelButton"/>・
    /// ②の <see cref="EarthquakePanelButton"/>・④の <see cref="TyphoonPanelButton"/> と
    /// 同じ形で、差分は名前・保存キー・クリック先だけ。
    ///
    /// **preferred 座標は①②④のボタンと同じ (8, 50) のままにしてある。これは手抜きでは
    /// ない。** <see cref="FreeSlotFinder"/> はそこが既に占有されていれば下へずらすので、
    /// 「同じ preferred から始めて、先に置かれたボタンを避ける」のがこの仕組みの
    /// 想定どおりの使い方である。①の全体レビューは、この探索が <c>SelfPrefix</c> による
    /// 一括除外のせいで**この MOD 自身のボタンを見落とす**欠陥を持っていたことを
    /// 見つけて撤去した（<c>FreeSlotFinder.SelfPrefix</c> の doc）。ここで新しい
    /// preferred 座標を発明すると、その修正が効いているかどうかを誰も確かめられなく
    /// なる上に、「他 MOD のボタンと重ならない」という当初の要件（ユーザーからの
    /// 明示要件）を、⑤が固定座標で再び壊すことになる。
    ///
    /// **①②④のボタンを除外してはいけない。** それらは実際に画面を占有している。
    /// </summary>
    public static class VolcanoPanelButton
    {
        private const string ButtonName = FreeSlotFinder.SelfPrefix + "VolcanoButton";
        ...
        /// 何もしない（既に設置済みなら撤去する）。
        ///
        /// **パネルも一緒に閉じる。** ボタンだけ先に消えると、開いたままのパネルを
        /// 閉じる手段が画面から無くなる（①のレビュー指摘）。
        /// </summary>
        ...
        public static void Install()
        {
            ...
            if (++_attempts > MaxAttempts)
            {
                ...
                Log.Warn("gave up looking for a UIView after "
                         + MaxAttempts + " attempts; volcano button not added");
                return;
            }
            ...
            try
            {
                ...
                if (view == null)
                {
                    Log.Diag("volcanoButton", "UIView not ready yet");
                    return;
                }
                ...
                button.text = Strings.VolcanoButtonLabel;
                button.tooltip = Strings.VolcanoButtonTooltip;
                ...
                button.eventClick += (c, e) => VolcanoPanel.Toggle();

                _button = button;
                Log.Info("volcano panel button installed at (" + pos.x + "," + pos.y + ")");
            }
            catch (System.Exception e)
            {
                // 例外で毎フレーム再突入しないよう、ここで諦める。
                _gaveUp = true;
                Log.Error("volcano panel button install failed", e);
            }
        }

        /// <summary>
        /// 保存済み座標があればそれを使い、無ければ <see cref="FreeSlotFinder"/> で探して保存する。
        /// 設定画面の「ボタン位置をリセット」は両方を -1 に戻すだけ（<see cref="ModSettings"/> 側）
        /// なので、ここでの再探索は次回のインストール（次のレベルロード）で自然に起きる。
        /// </summary>
        private static Vector2 ResolvePosition()
        {
            if (ModSettings.VolcanoButtonX.value >= 0 && ModSettings.VolcanoButtonY.value >= 0)
            {
                UsedSavedPosition = true;
                FoundFreeSlot = false;
                return new Vector2(ModSettings.VolcanoButtonX.value, ModSettings.VolcanoButtonY.value);
            }

            UsedSavedPosition = false;
            bool foundFree;
            // owner は null。ここは初回配置で、ボタンはこの呼び出しの後に生成される
            // （＝除外すべき「自分自身」がまだ画面に存在しない）。**①②④のボタンを
            ...
            if (!foundFree)
            {
                Log.Diag("volcanoButton", "no free slot found; using preferred position (may overlap)");
            }

            ModSettings.VolcanoButtonX.value = Mathf.RoundToInt(pos.x);
            ModSettings.VolcanoButtonY.value = Mathf.RoundToInt(pos.y);
            return pos;
        }
        ...
        public static void Remove()
        {
            ...
            // ★ ボタンが消えたらパネルも閉じる。閉じる手段だけ先に消えると、
            //    凍りついたパネルが消せなくなる（Tick の doc）。
            VolcanoPanel.Hide();

...
{
    /// <summary>
    /// 台風パネルを開閉するトグルボタン。①の <see cref="ForecastPanelButton"/> ・
    /// ②の <see cref="EarthquakePanelButton"/> と同じ形で、差分は名前・保存キー・
    /// クリック先だけ。
    ///
    ...
    /// </summary>
    public static class TyphoonPanelButton
    {
        private const string ButtonName = FreeSlotFinder.SelfPrefix + "TyphoonButton";
        ...
        public static void Install()
        {
            ...
            if (++_attempts > MaxAttempts)
            {
                ...
                Log.Warn("gave up looking for a UIView after "
                         + MaxAttempts + " attempts; typhoon button not added");
                return;
            }
            ...
            try
            {
                ...
                if (view == null)
                {
                    Log.Diag("typhoonButton", "UIView not ready yet");
                    return;
                }
                ...
                button.text = Strings.TyphoonTitle;
                button.tooltip = Strings.TyphoonTitle;
                ...
                button.eventClick += (c, e) => TyphoonPanel.Toggle();

                _button = button;
                Log.Info("typhoon panel button installed at (" + pos.x + "," + pos.y + ")");
            }
            catch (System.Exception e)
            {
                // 例外で毎フレーム再突入しないよう、ここで諦める。
                _gaveUp = true;
                Log.Error("typhoon panel button install failed", e);
            }
        }
        ...
        {
            if (ModSettings.TyphoonButtonX.value >= 0 && ModSettings.TyphoonButtonY.value >= 0)
            {
                UsedSavedPosition = true;
                FoundFreeSlot = false;
                return new Vector2(ModSettings.TyphoonButtonX.value, ModSettings.TyphoonButtonY.value);
            }
            ...
            if (!foundFree)
            {
                Log.Diag("typhoonButton", "no free slot found; using preferred position (may overlap)");
            }

            ModSettings.TyphoonButtonX.value = Mathf.RoundToInt(pos.x);
            ModSettings.TyphoonButtonY.value = Mathf.RoundToInt(pos.y);
            return pos;
        }
